...
function initRealtimeLogStream() {
  ...
  try {
    ...
    logEventSource.addEventListener('service-log', (e) => {
      try {
        const data = JSON.parse(e.data);
        if (typeof handleIncomingServiceLog === 'function') {
          handleIncomingServiceLog(data.serviceId, data.log);
        }
      } catch (err) {
        console.error('Error parsing SSE service-log:', err);
      }
    });

    logEventSource.addEventListener('service-status', (e) => {
      try {
        const data = JSON.parse(e.data);
        if (typeof handleServiceStatusChange === 'function') {
          handleServiceStatusChange(data.serviceId, data.status, data.pid);
        }
      } catch (err) {
        console.error('Error parsing SSE service-status:', err);
      }
    });
...
function switchAdminView(targetViewId) {
  const views = {
    ...
    viewServices: { title: 'Manajemen Service', element: viewServices, nav: navServices }
  };
  ...
  } else if (targetViewId === 'viewServices') {
    if (typeof fetchServicesList === 'function') fetchServicesList();
  }
}
...
function initAdminNav() {
  ...
  if (navServices) {
    navServices.addEventListener('click', () => switchAdminView('viewServices'));
  }
  if (btnSidebarDb) {
...
// ============================================================================
// MODUL MANAJEMEN SERVICE (Multi-Process & Program Manager)
// ============================================================================
let servicesList = [];
let activeConsoleServiceId = '';
const serviceLogsCache = {}; // serviceId -> Array of log objects

const badgeSidebarServiceCount = document.getElementById('badgeSidebarServiceCount');
...
function updateServiceMetrics() {
  ...
  if (badgeSidebarServiceCount) {
    badgeSidebarServiceCount.textContent = `${running} Aktif`;
    if (running > 0) {
      badgeSidebarServiceCount.style.background = 'rgba(34, 197, 94, 0.2)';
      badgeSidebarServiceCount.style.color = '#22c55e';
      badgeSidebarServiceCount.style.borderColor = 'rgba(34, 197, 94, 0.4)';
    } else {
      badgeSidebarServiceCount.style.background = 'rgba(59, 130, 246, 0.2)';
      badgeSidebarServiceCount.style.color = '#60a5fa';
      badgeSidebarServiceCount.style.borderColor = 'rgba(59, 130, 246, 0.3)';
    }
  }
}
...
function renderServicesList() {
  ...
  if (servicesList.length === 0) {
    servicesContainer.innerHTML = `
      <div class="empty-state" style="text-align: center; padding: 40px 20px; color: var(--text-dim); grid-column: 1 / -1;">
        <span style="font-size: 42px; display: block; margin-bottom: 12px;">⚙️</span>
        <h4 style="font-weight: 700; margin-bottom: 6px;">Belum Ada Service Terdaftar</h4>
        <p style="font-size: 13px; max-width: 420px; margin: 0 auto 16px;">
          Klik tombol "Tambah Service Baru" di atas untuk mendaftarkan script atau program yang ingin Anda jalankan di background.
        </p>
      </div>
    `;
    return;
    ...
    return `
      <div class="service-card ${cardClass}" id="serviceCard_${s.id}">
        <div class="service-card-header">
          ...
          <span class="badge ${statusBadgeClass}" style="font-size: 11px; padding: 3px 8px; font-weight: 700;">
            ${statusText}
          </span>
        </div>
        ...
        <div class="service-card-meta">
          ${isRunning ? `
            <span class="service-meta-tag" style="color: #22c55e; border-color: rgba(34, 197, 94, 0.3);">
        ...
        <div class="service-card-actions">
          ...
          <div class="service-actions-secondary">
            ...
            <button class="btn btn-xs btn-ghost" style="color: #ef4444;" onclick="triggerDeleteService('${s.id}', '${escapeHtml(s.name)}')" title="Hapus Service">
              🗑️
            </button>
          </div>
        </div>
